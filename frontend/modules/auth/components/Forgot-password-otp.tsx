...
import { fetchUser, verifyForgotPassword, verifyOtp } from '../services/auth.api';
...
const ForgotPasswordOtp: React.FC<OTPPageProps> = ({ userEmail = "user@example.com" }) => {
  ...
  const handleKeyDownOtp = (index: number, e: React.KeyboardEvent<HTMLInputElement>): void => {
  ...
  const handleVerifyForgotPassword = async (): Promise<void> => {
    ...
    console.log(otpCode,'otp type from frontend')
    if (otpCode.length === 6) {
      setIsLoading(true);
      setError('');
